...
class AliFileRepository extends FileRepository {
  ...
  Future<String?> getToken(FileItem file) async {
    return tokenService.getToken(file.mountId);
  }
  ...
  Future<List<FileItem>> listFile(FileItem file) async {
   return getToken(file).then((token) {
      ...
      return api
          ...
          .then(
            (res) => FileItem.toFileItemList(res.items, file.mountId ?? ""),
          );
    });
  }
  ...
  @override
  Future<List<FileItem>> rootFiles(String mountId) async {
    // DriveConfig? drive = await driveService.getDrive(name);
    return listFile(
      FileItem(id: "1", filename: "test", isDirectory: true, mountId: mountId),
    );
  }
  ...
  Future<VideoFile> videoPlayInfo(FileItem file) {
   ...
   return getToken(file).then((token) {
      ...
      return api.videoPlayInfo({
        "drive_id": "788219542",
        "file_id": file.id,
        "category":"live_transcoding"
        },token).then((res){
          
          print(res.video_preview_play_info.live_transcoding_task_list.first.url);
         return VideoFile(
            id: file.id,
            filename: file.filename,
            isDirectory: file.isDirectory,
            mountId: file.mountId,
            videoPlayUrl: res.video_preview_play_info.live_transcoding_task_list.first.url ,
          );
        }
